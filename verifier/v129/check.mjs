...
assert(/r\.height - RESERVED_TOP - RESERVED_BOTTOM/.test(ec), '可用高度扣除上下预留');
assert(/oy: RESERVED_TOP \+ \(availH - PH \* scale\) \/ 2/.test(ec), '游玩区垂直位置从预留顶起算');
...
// 4. 时间轴/信息面板半透明
assert(/g\.fillStyle = 'rgba\(12,12,17,0\.72\)'/.test(tl), '上时间轴 canvas 半透明底');
assert(/g\.fillStyle = 'rgba\(16,16,24,0\.7\)'/.test(tl), '下时间轴 canvas 半透明底');
assert(/bg-\[#151520\]\/70/.test(tl), '下时间轴容器背景半透明 /70');
assert(/bg-\[#0c0c11\]\/72/.test(tl), 'SelectionInfoPanel 背景半透明 /72');
