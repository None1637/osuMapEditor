...
// 4. 时间轴/信息面板半透明
assert(/g\.fillStyle = 'rgba\(12,12,17,0\.15\)'/.test(tl), '上时间轴 canvas 半透明底'); // v284 适配: 开关移除, 固定 0.15
assert(/fillStyle = 'rgba\(16,16,24,0\.15\)'/.test(tl), '下时间轴 canvas 半透明底'); // v284 适配: 同上
assert(/style=\{\{ background: 'rgba\(21,21,32,0\.1\)' \}\}/.test(tl), '下时间轴容器背景半透明 (v284 适配: 固定 0.1)');
assert(/style=\{\{ background: 'rgba\(12,12,17,0\.15\)' \}\}/.test(tl), 'SelectionInfoPanel 背景半透明 (v284 适配: 固定 0.15)');
