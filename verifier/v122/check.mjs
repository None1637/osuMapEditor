...
assert(/private stopAllHitVoices\(\) \{\s*for \(const e of this\.voiceLimiter\.drain\(\)\)/.test(store), 'stopAllHitVoices: drain 并停止全部 voice');
assert(/this\.stopAllHitVoices\(\); \/\/ v122: 引擎停止时 hitsound 一起停/.test(store), 'stopSource: 引擎停止时 hitsound 一起停');
assert(/pause\(\) \{[\s\S]{0,260}this\.stopSource\(\);/.test(store), 'pause 经 stopSource 停 hitsound');
