...
section('Timelines.tsx: 节拍刻度替代秒刻度');
{
  ...
  assert(/const down = k % meter === 0;/.test(src), '按 meter 判定小节首拍');
  assert(/const h = down \? 10 : 5;/.test(src), '首拍刻度更长 (10 vs 5)');
...
section('Timelines.tsx: 白色播放头 + 粉点加亮');
{
  ...
  assert(/g\.arc\(x\(o\.time\), mid, 1,/.test(bt), '粉点半径 (v159 改为 1 并落在中线上, 对齐 stable)');
}
