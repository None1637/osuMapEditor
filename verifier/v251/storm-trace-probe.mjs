// v251 探针4: 滚轮风暴后 hit 总线 16-20k HF 逐 50ms 时序 vs 对照 — 定位窗口内何处降质
...
try {
  ...
  await evalJs(`(() => {
    ...
    // voice 创建/停止记录 (trackVoice + ended)
    window.__vLog = [];
    const ck = s.clock;
    const origTV = s.trackVoice.bind(s);
    s.trackVoice = (buf, src, gain, startCtx, endCtx) => {
      const rec = { at: +(startCtx).toFixed(3), anchor: +ck.phaseStartSec.toFixed(3), end: null };
      src.addEventListener('ended', () => { rec.end = +s.actx.currentTime.toFixed(3); });
      window.__vLog.push(rec);
  ...
  const MEAS = `(async () => {
    ...
    window.__vMark = window.__vLog.length; // 窗口起点 (之后的 voice 属本窗口附近)
    const t0 = performance.now();
    while (performance.now() - t0 < 3000) await new Promise(r => setTimeout(r, 50));
    window.__tracing = false;
    return 1;
  })()`;
  console.log('== pass1 (对照)');
  await evalJs(`(() => { const s = window.__osuStore; s.pause(); s.seek(60000); s.play(); return 1; })()`);
  await sleep(200);
  await evalJs(MEAS);
  const t1 = await evalJs('window.__hfTrace.slice()');
  const v1 = await evalJs('window.__vLog.slice(window.__vMark)');
  await evalJs('window.__vLog = []');
  console.log('  hf:', JSON.stringify(t1));
  console.log('  voices:', JSON.stringify(v1));
  console.log('== pass2 (滚轮风暴)');
  await evalJs(`(() => { const s = window.__osuStore; s.pause(); s.seek(60000); s.play(); return 1; })()`);
  await sleep(200);
  await evalJs(`(async () => {
    const s = window.__osuStore;
  ...
  await evalJs(MEAS);
  const t2 = await evalJs('window.__hfTrace.slice()');
  const v2 = await evalJs(`(() => { const s = window.__osuStore;
    return { anchor: +s.clock.phaseStartSec.toFixed(3), voices: window.__vLog.filter(v => v.at > s.clock.phaseStartSec - 1.5) }; })()`);
  console.log('  hf:', JSON.stringify(t2));
  console.log('  voices:', JSON.stringify(v2));
