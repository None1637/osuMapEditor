...
section('seekWhilePlaying: 移除总线 dip (v226: 交叉淡变已被硬切换取代, 见 v226)');
{
  assert(!/bus\.gain\.(cancelScheduledValues|setTargetAtTime)/.test(seekBody), 'seek 不再触碰共享音乐总线增益');
  // v226: 以下交叉淡变形状断言已废止 (旧源 ~12ms 淡出/50ms 延迟停止/新源 ~10ms 淡入),
  //       保留接线断言 — per-source 增益结构不变
...
section('seekWhilePlaying: 变速支路只 dip tempoGain (v226: dip 形状改单次短窗, 见 v226)');
{
  assert(/tg\.cancelScheduledValues\(now\)/.test(seekBody), 'dip 仍只作用 tempoGain (支路增益, 非 musicGain)');
}
...
section('回归: 既有语义保留');
{
  assert(/now \+ 0\.00[34]/.test(seekBody), 'v193: 即时启动 (≤4ms 锚定; v226 改 3ms 切换时刻)');
