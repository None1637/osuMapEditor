...
export interface SpectroScroll { cv: HTMLCanvasElement; imgT0: number; win: number; w: number; h: number }
...
export function drawWave(g: CanvasRenderingContext2D, buf: AudioBuffer, t0: number, win: number, W: number, H: number, dpr: number) {
  ...
  g.fillStyle = WAVE_BG; g.fillRect(0, 0, W, H);
...
function renderSpectroStrip(cv: HTMLCanvasElement, buf: AudioBuffer, t0: number, win: number, x0: number, x1: number) {
  ...
  const px = img.data;
  for (let i = 3; i < px.length; i += 4) px[i] = SPECTRO_BG_ALPHA; // v105: 半透明黑底
  ...
  for (let cx = 0; cx < stripW; cx++) {
    ...
    for (let y = 0; y < H; y++) {
      ...
      px[o + 3] = Math.round(SPECTRO_BG_ALPHA + t * (255 - SPECTRO_BG_ALPHA)); // 静音处半透明, 强信号不透明
    }
  }
